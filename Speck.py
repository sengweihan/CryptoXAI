...
# starting off with the speck algo's input generation...
...
def real_differences_data(n, nr, diff=(0x0040,0)):
  ...
  plain0l = np.frombuffer(urandom(2*n),dtype=np.uint16); #
  plain0r = np.frombuffer(urandom(2*n),dtype=np.uint16);
  #apply input difference in order to establish a relationship between the plainl's.
  plain1l = plain0l ^ diff[0]; # set value
  plain1r = plain0r ^ diff[1]; # XOR with random generated value
  # print("l0", )
  num_rand_samples = np.sum(Y==0);
  
  #expand keys and repeatedly encrypts the 
  ks = expand_key(keys, nr);
  ...
  #apply blinding to the samples labelled as  to nake ciphertext indistinguishable
  ctdata0l[Y==0] = ctdata0l[Y==0] ^ k0; ctdata0r[Y==0] = ctdata0r[Y==0] ^ k1;
  ctdata1l[Y==0] = ctdata1l[Y==0] ^ k0; ctdata1r[Y==0] = ctdata1r[Y==0] ^ k1;
  #convert to input data for neural networks , 64 bit value
  X = convert_to_binary([ctdata0l, ctdata0r, ctdata1l, ctdata1r]);
  return(X,Y);