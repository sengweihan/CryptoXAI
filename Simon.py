...
class SimonCipher(object):
    ...
    __valid_setups = {
                        32: {64: (5, z0), 96: (6, z1), 128: (7, z2), 192: (8, z3)},
    ...
    def update_iv(self, new_iv):
        """
        Updates the current initialization vector
        """
        if new_iv:
            try:
    ...
    # Added by MCS5
    def make_simon_train_data( self , n , diff=(0x0040, 0) ):
        # generate labels
        ...
        # encrypt plaintexts to get ciphertexts
        ctdata0l = np.array( [ self.encrypt(val) for val in plain0l ] )
        ctdata0r = np.array( [ self.encrypt(val) for val in plain0r ] )
        ctdata1l = np.array( [ self.encrypt(val) for val in plain1l ] )
        ctdata1r = np.array( [ self.encrypt(val) for val in plain1r ] )
        X = self.convert_to_binary([ctdata0l, ctdata0r, ctdata1l, ctdata1r])
        return (X , Y)

    # Added by MCS5
    def real_differences_data( self , n , diff=(0x0040, 0) ):
        # generate labels
        Y = np.frombuffer(urandom(n), dtype=np.uint8)
        Y = Y & 1
        # generate plaintexts
